...
1. **Curated dataset** — a hand-vetted list of ~18 downtown/Penn
   Quarter/Chinatown/Wharf spots with coordinates, happy-hour windows, deals,
   ratings, price, and feature tags (rooftop, view, outdoor, oysters, …).
   Unlike a live scrape, it never breaks when a venue site blocks bots.
...
3. **Ranks** with a transparent composite score: venue rating + proximity +
   inherent feature desirability (rooftops/views weigh most) + a boost for
   features you ask for + a nudge for spots with a real standing happy hour.
...
```bash
python bot/happy_hour_bot.py                          # everything, ranked from the office
...
python bot/happy_hour_bot.py --happy-hour-only --max-walk 10
